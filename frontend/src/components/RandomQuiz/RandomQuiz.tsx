...
const RandomQuiz: React.FC = () => {
    ...
    const getQuestionPool = (): Question[] => {
        let pool: Question[] = [];
        if (courses.includes("precalculus"))
            pool = pool.concat(precalcQuestions);
        if (courses.includes("calculus1")) pool = pool.concat(calc1Question);
        if (courses.includes("linearAlgebra"))
            pool = pool.concat(linearAlgebra);
    ...
    const startQuiz = () => {
        ...
        if (pool.length === 0) {
            alert("No questions available. Please select at least one course.");
            return;
        }
...
function QuizCustomizationContainer({
    ...
    return (
        <form className="flex gap-5 p-3 pb-10">
            <div>
        ...
        */}
            ...
            <div>
                <label id="hard-level">Type of Questions</label>
                <select
                    name=""
                    id="hard-level"
