...
export const PolygonAngleExplorer = () => {
  ...
  const draw = () => {
    ...
    if (vertices.length >= 3) {
      ...
      ctx.fillText(`${interiorAngle.toFixed(1)}°`, curr.x + 25, curr.y - 10);
    }
    ...
    if (vertices.length >= 3) {
      ...
      ctx.fillText(`${exteriorAngle.toFixed(1)}°`, curr.x + 40, curr.y + 10);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      ...
      <p className="text-sm text-gray-600 mb-6">
        Drag vertices or adjust sides to explore interior, exterior, and central
        angles.
      </p>
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="bg-gray-50 p-5 rounded-lg space-y-3 min-w-64">
          <h3 className="font-semibold text-gray-800">Key Angles</h3>
          <div className="space-y-2 text-sm">
            <div>
              <strong className="text-blue-600">Interior Angle:</strong>{' '}
              <span className="text-black font-mono">{interiorAngle}°</span>
            </div>
            <div>
              <strong className="text-blue-600">Interior Sum:</strong>{' '}
              <span className="text-black font-mono">{interiorSum}°</span>
            </div>
            <div>
              <strong className="text-red-600">Exterior Angle:</strong>{' '}
              <span className="text-black font-mono">{exteriorAngle}°</span>
            </div>
            <div>
              <strong className="text-green-600">Central Angle:</strong>{' '}
              <span className="text-black font-mono">{centralAngle}°</span>
            </div>
          </div>
          ...
          <div className="pt-3 border-t">
            <p className="text-xs text-gray-600">
              ...
              💡 Exterior angles always sum to <strong>360°</strong>.
            </p>
          </div>
        </div>
      </div>
