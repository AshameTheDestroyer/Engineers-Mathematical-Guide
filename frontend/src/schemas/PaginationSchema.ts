...
export const CreatePaginationSchema = <T extends z.Schema>(schema: T) => {
    return z
        .object({
            items: z.array(schema, "required"),
            page: z.number("required").nonnegative("nonnegative"),
            total: z.number("required").nonnegative("nonnegative"),
            limit: z.number("required").nonnegative("nonnegative"),
        })
